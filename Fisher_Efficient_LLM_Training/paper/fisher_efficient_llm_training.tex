...
\title{Memory Efficient LLM Training with Random Subspace Stochastic Natural Gradient Descent}
...
\begin{tikzpicture}[node distance=2cm and 3cm]
   ...
   \node (imgdecode) [process, below of=heira] {Image Decoder};
   \node (llm) [process, below of=imgdecode ] {LLM};
   \node (promptimg) [process, left of=llm] {Prompt};
   \node (ocr) [process, below of=llm, xshift=-2cm, yshift=-1cm] {OCR};
   \node (qa) [process, below of=llm, xshift=2cm, yshift=-1cm] {Question and Answer};
   \node (layout) [process, below of=llm, xshift=6cm, yshift=-1cm] {Layout Detection};
   \node (ner) [process, below of=llm, xshift=10cm, yshift=-1cm] {Named Entity Recognition};

   % Arrows
   \draw [arrow] (start) -- (heira);
   \draw [arrow] (heira) -- (imgdecode);
   \draw [arrow] (imgdecode) -- (llm);
   \draw [arrow] (promptimg) -- (llm);
   \draw [arrow] (llm) -- (ocr);
   \draw [arrow] (llm) -- (qa);
